...
class DoctorLeaveController extends Controller
{
    ...
    public function create()
    {
        ...
        $sameDepartmentDoctors = \App\Models\Doctor::where('department_id', $currentDoctor->department_id)
    ...
    public function store(Request $request)
    {
        ...
        $doctorId = $doctor->id;
        $isEmergency = $request->has('urgent'); // theo DB bạn: cột tên là 'urgent'
        ...
        // 3. Chỉ được nghỉ 1 lần mỗi tháng (đơn thường)
        ...
        if ($hasApproved && !$isEmergency) {
            return back()->withErrors(['start_date' => 'Bạn chỉ được nghỉ 1 lần mỗi tháng (đơn thường).'])->withInput();
        }

        // 4. Trùng lịch làm việc
        ...
        // 7. Nếu nghỉ đột xuất → xử lý thay bác sĩ
        if ($isEmergency) {
            $period = Carbon::parse($request->start_date)->daysUntil($request->end_date->copy()->addDay());
            foreach ($period as $date) {
                $this->handleUrgentReplacementOrCancel($doctor, $date);
            }
        }
    ...
    protected function handleUrgentReplacementOrCancel($doctor, Carbon $leaveDay)
    {
        $appointments = Appointment::where('doctor_id', $doctor->id)
            ->whereDate('date', $leaveDay->toDateString())
            ->get();
        ...
        foreach ($appointments as $appointment) {
            $replacement = $sameSpecialtyDoctors->first(function ($doc) use ($leaveDay) {
                return !DoctorLeave::where('doctor_id', $doc->id)
            ...
            if ($replacement) {
                $appointment->doctor_id = $replacement->id;
                $appointment->note = 'Bác sĩ đã nghỉ đột xuất, bạn được chuyển sang bác sĩ khác.';
                $appointment->save();

                // Gửi thông báo
                $appointment->user->notify(new \App\Notifications\AppointmentReassigned($appointment));
            } else {
                $appointment->status = 'cancelled';
                $appointment->note = 'Cuộc hẹn bị hủy vì bác sĩ nghỉ đột xuất và không có ai thay thế.';
                $appointment->save();

                $appointment->user->notify(new \App\Notifications\AppointmentCancelledWithSuggestion($appointment));
            }
        }
    }
};
