...
@section('content')
        ...
        <form action="{{ route('admin.prescriptions.store') }}" method="POST">
            ...
            <div id="medicine-list">
                <div class="medicine-item border p-2 mb-2">
                    ...
                    <input type="number" name="medicines[0][quantity]" class="form-control mb-2" placeholder="Số lượng"
                        min="1" required>
    ...
    <template id="medicine-template">
        <div class="medicine-item border p-2 mb-2">
            <select name="medicines[__index__][medicine_id]" class="form-control mb-2" required>
                <option value="">-- Chọn thuốc --</option>
                @foreach ($medicines as $med)
                    <option value="{{ $med->id }}">{{ $med->name }} ({{ $med->unit }})</option>
                @endforeach
            ...
            <input type="number" name="medicines[__index__][quantity]" class="form-control mb-2" placeholder="Số lượng"
                min="1" required>

            <textarea name="medicines[__index__][usage_instructions]" class="form-control" placeholder="Hướng dẫn sử dụng"></textarea>
        </div>
    </template>
