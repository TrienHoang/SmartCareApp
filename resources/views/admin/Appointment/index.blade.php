        ...
        <div class="filter-section mb-4">
            <form method="GET" action="{{ route('admin.appointments.index') }}" class="row g-3">
                <div class="col-md-3">
                    ...
                    <input type="text" class="form-control" name="search" value="{{ request('search') }}" placeholder="Tên hoặc số điện thoại...">
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <input type="date" class="form-control" name="date_from" value="{{ old('date_from', request('date_from')) }}">
                </div>

                <div class="col-md-2">
                    <label class="form-label">Đến ngày</label>
                    <input type="date" class="form-control" name="date_to" value="{{ old('date_to', request('date_to')) }}">
                </div>
                ...
                <table class="table table-hover">
                    <thead class="table-dark">
                        <tr>
                            <th>STT</th>
                            <th>
                                <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'patient.full_name', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc']) }}" class="text-white text-decoration-none">
                                    Bệnh nhân
                                    @if (request('sort_by') == 'patient.full_name')
                                        <i class="bx bx-{{ request('sort_order') == 'asc' ? 'up' : 'down' }}-arrow-alt"></i>
                                    @endif
                                </a>
                            </th>
                            <th>Bác sĩ</th>
                            <th>Phòng/Khoa</th>
                            <th>
                                <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'appointment_time', 'sort_order' => request('sort_order') == 'asc' ? 'desc' : 'asc']) }}" class="text-white text-decoration-none">
                                    Thời gian
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex flex-column">
                                        ...
                                        <small class="text-muted">{{ \Carbon\Carbon::parse($appointment->end_time)->format('H:i') }} (Dự kiến)</small>
                                    </div>
                                </td>
                                <td>
                                    @php
                                ...
                                <td>
                                    @if (($appointment->payment && $appointment->payment->status === 'paid') || ($appointment->order && $appointment->order->status === 'completed'))
                                ...
                                <td>
                                    <div class="table-actions">
                                        <a href="{{ route('admin.appointments.show', $appointment->id) }}" class="btn btn-sm btn-outline-primary" title="Xem chi tiết">
                                            <i class="bx bx-show"></i>
                                        </a>

                                        @if ($appointment->status != 'completed' && $appointment->status != 'cancelled')
                                            <a href="{{ route('admin.appointments.edit', $appointment->id) }}" class="btn btn-sm btn-outline-warning" title="Chỉnh sửa">
                                                <i class="bx bx-edit"></i>
                                            </a>
                                        @endif

                                        @if ($appointment->status === 'confirmed')
                                            <button class="btn btn-sm btn-outline-success" onclick="updateStatus({{ $appointment->id }}, 'completed')" title="Hoàn thành">
                                                <i class="bx bx-check-double"></i>
                                            </button>
                                        @endif

                                        @if (optional($appointment->payment)->status !== 'paid')
                                            <form action="{{ route('admin.appointments.pay', $appointment->id) }}" method="POST" class="d-inline">
                                                @csrf
                                        ...
                                        @if (in_array($appointment->status, ['pending', 'confirmed']))
                                            <button class="btn btn-sm btn-outline-danger" onclick="showCancelModal({{ $appointment->id }})" title="Hủy lịch hẹn">
            ...
            @if ($appointments->hasPages())
                <div class="card-footer">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="text-muted">
                            Hiển thị {{ $appointments->firstItem() }} - {{ $appointments->lastItem() }} trong tổng số {{ $appointments->total() }} bản ghi
                        </div>
                ...
                <form id="statusForm" method="POST">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="note" rows="3" placeholder="Nhập ghi chú về việc thay đổi trạng thái..."></textarea>
                        </div>
                    </div>
