...
@section('title', 'Chỉnh sửa thông tin Bác sĩ')
                ...
                <div class="col-12">
                    <div class="d-flex align-items-center mb-3">
                        <div class="icon-circle bg-primary me-3 ">
                            <i class="bx bx-edit text-white "></i> {{-- Icon for Edit Doctor --}}
                        </div>
                        <div>
                            <h2 class="content-header-title mb-0 text-primary fw-bold">Chỉnh sửa Bác sĩ</h2>
                            <p class="text-muted mb-0">Cập nhật thông tin chi tiết của bác sĩ</p>
                        </div>
                    </div>
                    <div class="breadcrumb-wrapper col-12">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb bg-transparent p-0">
                                ...
                                <li class="breadcrumb-item active text-primary fw-semibold">Chỉnh sửa</li>
                            </ol>
                        </nav>
                    </div>
                </div>
            ...
            <div class="card border-0 shadow rounded-4">
                <div class="card-header bg-primary text-white rounded-top-4 p-4">
                    <h3 class="card-title mb-0">
                        <i class="bx bx-edit me-2"></i> Chỉnh sửa thông tin cho: {{ $doctor->user->full_name ?? $doctor->user->username ?? 'Bác sĩ' }}
                    </h3>
                </div>
                <div class="card-body p-4 bg-light">
                    <form action="{{ route('admin.doctors.update', $doctor->id) }}" method="POST">
                        ...
                        {{-- Không sửa tên người dùng ở đây, chỉ hiển thị --}}
                        <div class="mb-3">
                            <label class="form-label fw-semibold">
                                <i class="bx bx-user me-2"></i> Người dùng
                            </label>
                            <input type="text" class="form-control"
                                value="{{ $doctor->user->full_name ?? $doctor->user->username ?? 'Không có' }}" disabled>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="biography" id="biography" class="form-control @error('biography') is-invalid @enderror" rows="4">{{ old('biography', $doctor->biography) }}</textarea>
                            @error('biography')
                                <div class="invalid-feedback d-flex align-items-center mt-1">
                                    <i class="bx bx-info-circle me-1"></i> {{ $message }}
                                </div>
                        ...
                        <div class="d-flex justify-content-center gap-3 mt-4">
                            <button type="submit" class="btn btn-primary btn-lg px-5 py-3 rounded-3 shadow-sm">
                                <i class="bx bx-check me-2"></i> Cập nhật
                            </button>
