                ...
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <div class="d-flex align-items-center mb-3">
                            ...
                            <div>
                                <h2 class="content-header-title mb-0 text-primary font-weight-bold">Quản lý Câu hỏi thường
                                    gặp</h2>
                                <p class="text-muted mb-0">Quản lý và theo dõi tất cả câu hỏi thường gặp trong hệ thống</p>
                            </div>
                        </div>
                            ...
                            <div class="d-flex align-items-center">
                                <div class="avatar bg-rgba-white mr-2">
                                    <div class="avatar-content">
                                        <i class="bx bx-show font-medium-5"></i>
                                    </div>
                                </div>
                                <div>
                                    <h4 class="text-white mb-0">{{ $faqs->where('is_active', 1)->count() }}</h4>
                                    <small class="text-white">Đang hiển thị</small>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card gradient-card bg-gradient-warning">
                        <div class="card-body text-white">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h4 class="text-white mb-0">{{ $faqs->where('is_active', 0)->count() }}</h4>
                                    <small class="text-white">Đang ẩn</small>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card gradient-card bg-gradient-info">
                        <div class="card-body text-white">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h4 class="text-white mb-0">{{ $faqs->count() }}</h4>
                    ...
                    <div class="card gradient-card bg-gradient-danger">
                        <div class="card-body text-white">
                            <div class="d-flex align-items-center">
                                ...
                                <div>
                                    <h4 class="text-white mb-0">{{ count($categories) }}</h4>
                    ...
                    <div class="filter-section bg-light p-4 border-bottom">
                        <form action="{{ route('admin.faqs.index') }}" method="GET" class="filter-form">
                            <div class="row align-items-end">
                                <div class="col-lg-3 col-md-6 mb-2">
                                    ...
                                    <div class="input-group">
                                        <input type="text" name="keyword" class="form-control border-left-0"
                                            placeholder="Tìm câu hỏi..." value="{{ request('keyword') }}">
                                    </div>
                                </div>
                                <div class="col-lg-3 col-md-6 mb-2">
                                    <label class="form-label font-weight-semibold">
                                    ...
                                    <select name="service_category_id" class="form-control custom-select">
                                        <option value="">Tất cả danh mục</option>
                                        @foreach ($categories as $id => $name)
                                            <option value="{{ $id }}"
                                                {{ request('service_category_id') == $id ? 'selected' : '' }}>
                                                {{ $name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-lg-3 col-md-6 mb-2">
                                    <label class="form-label font-weight-semibold">
                                        <i class="bx bx-show mr-1 text-success"></i>Trạng thái
                                    </label>
                        ...
                        <table class="table table-hover table-modern mb-0">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th class="border-top-0">
                                        <i class="mr-1"></i>Câu hỏi
                                    </th>
                                    <th class="border-top-0">
                                        <i class="mr-1"></i>Danh mục
                                    </th>
                                    <th class="border-top-0">
                            ...
                            <tbody>
                                @forelse ($faqs as $faq)
                                    <tr class="faq-row" data-id="{{ $faq->id }}">
                                        <td>
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" class="custom-control-input faq-checkbox"
                                                    id="faq-{{ $faq->id }}" value="{{ $faq->id }}">
                                                <label class="custom-control-label"
                                                    for="faq-{{ $faq->id }}"></label>
                                            </div>
                                        </td>
                                        <td class="font-weight-bold text-primary">#{{ $faq->id }}</td>
                                        <td>
                                            <div class="faq-title">
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm" role="group">
                                                ...
                                                <form action="{{ route('admin.faqs.toggle-status', $faq->id) }}"
                                                    ...
                                                    onsubmit="return confirm('Bạn có chắc muốn {{ $faq->is_active ? 'ẩn' : 'hiển thị' }} câu hỏi này?');">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit"
                                                        class="btn btn-outline-{{ $faq->is_active ? 'secondary' : 'success' }}"
                                                        data-toggle="tooltip"
                                                        title="{{ $faq->is_active ? 'Ẩn câu hỏi' : 'Hiển thị câu hỏi' }}">
                                                        <i class="bx bx-{{ $faq->is_active ? 'hide' : 'show' }}"></i>
                                                    </button>
                                                </form>
                                    ...
                                    <tr>
                                        <td colspan="6" class="text-center py-5">
                                            <div class="empty-state">
                                                <i class="bx bx-help-circle text-muted" style="font-size: 48px;"></i>
                                                <h5 class="mt-3 text-muted">Không có câu hỏi nào</h5>
                                                <p class="text-muted">Chưa có câu hỏi thường gặp nào được tạo hoặc không
                                                    tìm thấy kết quả phù hợp.</p>
                                                <a href="{{ route('admin.faqs.create') }}" class="btn btn-primary">
                                                    <i class="bx bx-plus mr-1"></i>Tạo câu hỏi đầu tiên
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="pagination-info">
                                    <small class="text-muted">
                                        Hiển thị {{ $faqs->firstItem() }} - {{ $faqs->lastItem() }}
                                        trong tổng số {{ $faqs->total() }} kết quả
                                    </small>
                                </div>
    ...
    <script>
        ...
        function toggleStatus(id, currentStatus) {
            ...
            Swal.fire({
                ...
                text: `Bạn có chắc chắn muốn ${statusText} câu hỏi này?`,
                icon: 'question',
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = `/admin/faqs/${id}/toggle-status`;
