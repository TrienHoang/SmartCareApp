        ...
        <div class="container mx-auto px-4 py-6 lg:py-10">
            ...
            <nav class="mb-8" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2 text-sm">
                    ...
                    <li>
                        <span class="text-gray-700 font-medium">{{ $post->serviceCategory->name ?? 'Chuyên mục' }}</span>
                    </li>
                </ol>
            </nav>
            ...
            <div class="mb-8">
                <h1 class="text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-gray-900 leading-tight mb-4 tracking-tight">
                    {{ $post->title }}
                </h1>
            ...
            <div class="flex flex-wrap items-center gap-6 text-sm">
                @if($post->created_at)
                    <div class="flex items-center bg-gray-100 rounded-full px-4 py-2 hover:bg-gray-200 transition-colors">
                ...
                @if($post->category)
                    <div class="flex items-center">
                        <div class="bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-3 rounded-full text-sm font-bold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-300">
        ...
        <div class="max-w-full mx-auto">
            ...
            @if($post->thumbnail)
                <div class="mb-16 group">
                    <div class="relative overflow-hidden rounded-3xl shadow-2xl hover:shadow-3xl transition-all duration-500">
                        <div class="absolute inset-0 bg-gradient-to-br from-blue-600/20 via-transparent to-purple-600/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10"></div>
                        ...
                        @if($post->image_caption ?? false)
                            <div class="absolute bottom-4 left-4 right-4 bg-black/60 backdrop-blur-sm text-white p-4 rounded-xl">
                        ...
                        <div class="space-y-2 text-xs">
                            @if($post->created_at)
                                <div class="flex items-center justify-between">
                                    ...
                                    <span class="font-semibold text-gray-900">{{ $post->created_at->format('d/m/Y') }}</span>
                                </div>
                            @endif
                            @if($post->author)
                                <div class="flex items-center justify-between">
                        ...
                        <div class="grid grid-cols-1 gap-2">
                            ...
                            <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($post->title) }}" 
                               ...
                               class="flex items-center justify-center bg-[#1DA1F2] hover:bg-[#1A91DA] text-white p-2 rounded-lg transition-all duration-300 group text-xs">
                                <svg class="w-4 h-4 mr-2 group-hover:scale-110 transition-transform duration-300" fill="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <a href="{{ route('posts.show', $related->slug) }}" class="group block">
                                        <div class="flex space-x-2">
                                            ...
                                            <div class="flex-1 min-w-0">
                                                <h4 class="text-xs font-semibold text-gray-900 group-hover:text-blue-600 transition-colors duration-300 line-clamp-2 leading-tight">
                                                    {{ $related->title }}
                                                </h4>
                            ...
                            <div class="mb-12">
                                ...
                                <div class="flex flex-wrap gap-3">
                                    @foreach($post->tags as $tag)
                                        <span class="group bg-white hover:bg-blue-50 border-2 border-gray-200 hover:border-blue-300 text-gray-700 hover:text-blue-700 px-6 py-3 rounded-full text-sm font-semibold transition-all duration-300 cursor-pointer transform hover:-translate-y-1 hover:shadow-lg">
                                            <span class="group-hover:scale-110 transition-transform duration-300 inline-block">#{{ $tag }}</span>
                                        </span>
                                    @endforeach
                                </div>
                            </div>
                            ...
                            <div class="flex flex-wrap gap-4">
                                ...
                                <a href="https://twitter.com/intent/tweet?url={{ urlencode(request()->url()) }}&text={{ urlencode($post->title) }}" 
                                   ...
                                   class="group flex items-center bg-[#1DA1F2] hover:bg-[#1A91DA] text-white px-8 py-4 rounded-2xl transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                                    <svg class="w-5 h-5 mr-3 group-hover:scale-110 transition-transform duration-300" fill="currentColor" viewBox="0 0 24 24">
                                        <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
                                    </svg>
                                    Twitter
                                </a>
            ...
            <div class="mt-16 flex flex-col lg:flex-row gap-6">
                <a href="{{ route('client.news.index') }}" 
                   class="group flex items-center justify-center bg-white hover:bg-gray-50 text-gray-800 border-2 border-gray-200 hover:border-gray-300 px-10 py-5 rounded-2xl transition-all duration-300 font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-1 flex-1">
                    <svg class="w-6 h-6 mr-3 group-hover:-translate-x-2 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function copyToClipboard(text) {
    navigator.clipboard.writeText(text).then(function() {
        ...
        button.classList.remove('bg-gradient-to-r', 'from-green-500', 'to-emerald-600', 'hover:from-green-600', 'hover:to-emerald-700');
        ...
        setTimeout(() => {
            ...
            button.classList.add('bg-gradient-to-r', 'from-green-500', 'to-emerald-600', 'hover:from-green-600', 'hover:to-emerald-700');
        }, 2000);
    }).catch(function() {
...
<style>
@media print {
    .no-print, .lg\:col-span-4, nav, .sticky { display: none !important; }
