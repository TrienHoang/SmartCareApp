                        ...
                        @forelse($paymentHistories as $payment)
                            <tr>
                                <td class="text-center">
                                    {{ \Carbon\Carbon::parse($payment->payment_date)->format('d/m/Y H:i') }}</td>
                                <td class="text-center">
                                    <span
                                        class="badge bg-{{ $payment->payment_method === 'cash' ? 'secondary' : ($payment->payment_method === 'card' ? 'info text-dark' : 'primary') }}">
                                        {{ ucfirst($payment->payment_method) }}
                                    </span>
                                </td>
                                <td class="text-end text-success">{{ number_format($payment->amount, 0, ',', '.') }} VNĐ
                                </td>
                                <td class="text-center">
                                    @if ($payment->payment_method === 'Wallet_refund' || $payment->payment_method === 'wallet_refund')
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-4">
                                    <span class="no-data"><i class="fas fa-money-bill-wave-slash me-1"></i> Không có dữ liệu
                                        thanh toán.</span>
                                </td>
                            </tr>
                        @endforelse
