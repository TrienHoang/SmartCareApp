...
@section('title', 'Chi Tiết Đơn Thuốc #' . $prescription->id) {{-- Đặt tiêu đề động --}}
    ...
    <div class="min-h-screen bg-gray-50 py-12"> {{-- Bao bọc toàn bộ nội dung trong một container chính --}}
        <div class="container mx-auto px-4">
            <div class="flex flex-col lg:flex-row gap-8">
                ...
                {{-- Main Content for Prescription Details --}}
                <div class="lg:w-3/4">
                    <div class="bg-white rounded-xl shadow-lg p-8 mb-8">
                        ...
                        <div class="flex items-center justify-between mb-6 border-b pb-4">
                            <div class="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600 text-sm">Mã đơn: <span class="font-semibold text-blue-600">#{{ $prescription->id }}</span></p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 border-b pb-6">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-900">{{ \Carbon\Carbon::parse($prescription->created_at)->format('d/m/Y - H:i') }}</p>
                            </div>
                        ...
                        <h3 class="text-xl font-bold text-gray-900 mb-6 flex items-center justify-between">
                            Danh Sách Thuốc
                            <span class="bg-blue-600 text-white text-sm font-semibold px-3 py-1 rounded-full">{{ count($prescription->prescriptionItems) }} thuốc</span>
                        </h3>
                        ...
                        <div class="space-y-4">
                            @forelse($prescription->prescriptionItems as $index => $item)
                                <div class="flex items-start bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200">
                                    ...
                                    <div class="flex-grow">
                                        <h4 class="text-lg font-semibold text-gray-900 mb-1">{{ $item->medicine->name }}</h4>
                                        ...
                                        <div class="flex flex-wrap gap-4 text-sm">
                                            <div class="flex items-center space-x-1">
                                                ...
                                                <span class="px-2 py-0.5 bg-green-100 text-green-800 rounded-md font-semibold">{{ $item->medicine->dosage }}</span>
                                            </div>
                                            <div class="flex items-center space-x-1">
                                                <span class="font-medium text-gray-500">Đơn vị:</span>
                                                <span class="px-2 py-0.5 bg-yellow-100 text-yellow-800 rounded-md font-semibold">{{ $item->medicine->unit }}</span>
                                            </div>
                                            <div class="flex items-center space-x-1">
                                                <span class="font-medium text-gray-500">Số lượng:</span>
                                                <span class="px-2 py-0.5 bg-purple-100 text-purple-800 rounded-md font-semibold">{{ $item->quantity }}</span>
                                            </div>
                                            @if ($item->notes)
                                                <div class="flex items-center space-x-1">
                    ...
                    <div class="bg-white rounded-xl shadow-lg p-8 mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                            <a href="{{ route('client.prescriptions.index') }}" class="inline-flex items-center justify-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                <i data-lucide="arrow-left" class="w-4 h-4 mr-2"></i> Quay lại
                            </a>
                            <button onclick="window.print()" class="inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                                <i data-lucide="printer" class="w-4 h-4 mr-2"></i> In đơn
                            </button>
                            <button onclick="downloadPrescription()" class="inline-flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                ...
                                <p class="text-xl font-bold text-blue-600">{{ count($prescription->prescriptionItems) }}</p>
                            </div>
                            <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                <p class="text-gray-600 text-sm">Tổng số lượng:</p>
                                <p class="text-xl font-bold text-blue-600">{{ $prescription->prescriptionItems->sum('quantity') }}</p>
                            </div>
                            <div class="p-4 bg-gray-50 rounded-lg border border-gray-200">
                                <p class="text-gray-600 text-sm">Ngày kê:</p>
                                <p class="text-xl font-bold text-blue-600">{{ \Carbon\Carbon::parse($prescription->created_at)->format('d/m/Y') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    @push('styles')
        <style>
            ...
            .prescription-detail-container {
                background: #f8fafc;
                min-height: auto; /* Allow the parent min-h-screen to control overall height */
            ...
            */
            ...
            @media print {
                ...
                #app { /* Assuming #app is your main container */
                    display: block !important;
                }
                .min-h-screen, .container, .flex {
                    display: block !important;
                    width: 100% !important;
                    padding: 0 !important;
                    margin: 0 !important;
                ...
                .lg\:w-1\/4, .lg\:w-3\/4 { /* Hide sidebar and make main content full width */
                    display: none !important;
                    width: 100% !important;
                }
                .bg-gray-50, .shadow-lg, .border, .rounded-xl {
                ...
                .status-badge {
                    display: none !important; /* Hide status badge in print */
                }
                .sidebar-card, .actions-grid, .summary-grid {
                    display: none !important;
                }
                .col-lg-4 {
                    display: none !important;
    ...
    @push('scripts')
        <script>
            ...
            function downloadPrescription() {
                ...
                window.print();
                alert('Tính năng tải xuống PDF đang được phát triển. Vui lòng sử dụng chức năng "In" của trình duyệt để lưu trang này dưới dạng PDF.');
